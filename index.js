...
app.get("/api/products/updates", isAuth, async (req, res) => {
    try {
        ...
        if (!lastSync)
            return res.status(400).json({ error: "lastSync required" });
...
app.post("/api/upload", upload.single("image"), async (req, res) => {
    try {
        if (!req.file || !req.file.buffer) {
            return res
                .status(400)
                .json({ ok: false, message: "Aucun fichier reçu" });
        }
...
app.delete("/api/products/:id", async (req, res) => {
    ...
    try {
        ...
        if (!deleted) {
            return res
                .status(404)
                .json({ ok: false, message: "Produit non trouvé" });
        }
...
app.put("/api/vente/:id", async (req, res) => {
    try {
        ...
        if (!updatedSale)
            return res
                .status(404)
                .json({ ok: false, message: "Vente introuvable ❌" });
...
app.get("/api/customers/:id", async (req, res) => {
    ...
    try {
        ...
        if (!customer)
            return res
                .status(404)
                .json({ ok: false, error: "Client non trouvé" });
        res.json(customer);
    } catch (err) {
